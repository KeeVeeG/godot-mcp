...
export function registerTestingTools(server, bridge) {
    // 1. run_test_scenario
    server.registerTool('run_test_scenario', {
    ...
    // 2. assert_node_state
    server.registerTool('assert_node_state', {
    ...
    }, async (args) => callGodot(bridge, 'testing/assert_state', args));
    // 3. assert_screen_text
    server.registerTool('assert_screen_text', {
    ...
    }, async (args) => callGodot(bridge, 'testing/assert_screen_text', args));
    // 4. run_stress_test
    ...
    }, async (args) => callGodot(bridge, 'testing/stress_test', args));
    // 5. get_test_report
