...
export function registerAnimationTools(server, bridge) {
    ...
    }, async (args) => callGodot(bridge, 'animation/list', args));
    ...
    server.registerTool('set_animation_keyframe', {
    ...
    }, async (args) => callGodot(bridge, 'animation/set_keyframe', args));
    // 5. get_animation_info — {player_path: string, animation: string} -> detailed info
    server.registerTool('get_animation_info', {
    ...
    }, async (args) => callGodot(bridge, 'animation/get_info', args));
    // 6. remove_animation — {player_path: string, animation: string} -> success
    server.registerTool('remove_animation', {
    ...
    }, async (args) => callGodot(bridge, 'animation/remove', args));
    // 7. create_animation_tree — {path: string, properties?: Record<string,any>} -> success
    server.registerTool('create_animation_tree', {
    ...
    }, async (args) => callGodot(bridge, 'animation/create_tree', args));
    // 8. get_animation_tree_structure — {path: string} -> tree structure
    server.registerTool('get_animation_tree_structure', {
    ...
    }, async (args) => callGodot(bridge, 'animation/get_tree_structure', args));
    // 9. set_tree_parameter — {path: string, parameter: string, value: any} -> success
    server.registerTool('set_tree_parameter', {
