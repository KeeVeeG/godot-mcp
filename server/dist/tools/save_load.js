...
export function registerSaveLoadTools(server, bridge) {
  // 1. save_game_state
  server.registerTool(
    'save_game_state',
    {
    ...
    async (args) => callGodot(bridge, 'save_game_state', args),
  );
  // 2. load_game_state
  server.registerTool(
    'load_game_state',
    ...
    async (args) => callGodot(bridge, 'load_game_state', args),
  );
  // 3. list_save_files
  server.registerTool(
    'list_save_files',
  ...
  server.registerTool(
    ...
    async (args) => callGodot(bridge, 'delete_save_file', args),
  );
  ...
  server.registerTool(
    ...
    async (args) => callGodot(bridge, 'compare_save_states', args),
  );
}
