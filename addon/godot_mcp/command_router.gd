...
## Handles both {success: true/false} and legacy {result:}/{error:} formats.
func route_request(method_name: String, params: Dictionary) -> Dictionary:
	...
	if result is Dictionary:
		...
		# Handle {success: true/false} format from command modules
		if dict.has("success"):
			if dict["success"]:
				# Success: wrap all data under "result" key for plugin compatibility
				var data: Dictionary = dict.duplicate()
				data.erase("success")
				return {"result": data}
			else:
				# Error: wrap in standard error format
				return {"error": {"code": -1, "message": dict.get("error", "Unknown error")}}
		# Handle {error: "string"} format (no success key)
